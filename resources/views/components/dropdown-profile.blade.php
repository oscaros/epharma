        ...
        <ul>
            ...
            <li>
                <form method="POST" action="{{ route('logout') }}" x-data>
                    ...
                    <a class="font-medium text-sm text-indigo-500 hover:text-indigo-600 dark:hover:text-indigo-400 flex items-center py-1 px-3 signout" id="signout"
                        href="{{ route('logout') }}"
                        @click.prevent="$root.submit();"
                        {{-- clear session on log out --}}
                        x-on:submit="$wire.clearSession()"
                        @focus="open = true"
                        @focusout="open = false"
                    >
                        {{ __('Sign Out') }}
