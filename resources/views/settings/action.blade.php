<div class="flex align-items-center list-business-action">
    ...
    {{-- @if (Auth::user()->role->permissions->contains('name', 'View Settings')) --}}
    <!-- View Action -->
    <a class="btn btn-sm btn-icon btn-info" data-bs-toggle="tooltip" title="View Stock" href="{{ route('settings.show', $id) }}">
    ...
    {{-- @if (Auth::user()->role->permissions->contains('name', 'Edit Settings')) --}}

    <!-- Edit Action -->
    <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="tooltip" title="Edit Business" href="{{ route('settings.edit', $id) }}">
        <span class="btn-inner">
            <svg width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <!-- Delete Action -->
    {{-- @if (Auth::user()->role->permissions->contains('name', 'Delete Settings')) --}}
    <a class="btn btn-sm btn-icon btn-danger" onclick="confirmDelete('{{ $id }}')" data-bs-toggle="tooltip" title="Delete Stock" href="#">
        <span class="btn-inner">
    ...
    <form action="{{ route('settings.destroy', $id) }}" id="Stock-delete-{{ $id }}" method="post">
        @method('delete')
        @csrf
    </form>
        ...
        function confirmDelete(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.getElementById('stock-delete-' + id).submit();
                }
            });
        }
    </script>
