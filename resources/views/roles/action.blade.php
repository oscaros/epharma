<div class="flex align-items-center list-business-action">
    <!-- View Action -->
    @if (in_array('View Roles', json_decode(Auth::user()->role->permissions)))
    <a class="btn btn-sm btn-icon btn-info" data-bs-toggle="tooltip" title="View Role" href="{{ route('roles.show', $id) }}">
    ...
    <!-- Edit Action -->
    @if (in_array('Edit Roles', json_decode(Auth::user()->role->permissions)))
    <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="tooltip" title="Edit Role" href="{{ route('roles.edit', $id) }}">
        <span class="btn-inner">
            <svg width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <!-- Delete Action -->
    @if (in_array('Delete Roles', json_decode(Auth::user()->role->permissions)))
    <a class="btn btn-sm btn-icon btn-danger" onclick="confirmDelete('{{ $id }}')" data-bs-toggle="tooltip" title="Delete Role" href="#">
        <span class="btn-inner">
    ...
    <form action="{{ route('roles.destroy', $id) }}" id="role-delete-{{ $id }}" method="post">
        @method('delete')
        @csrf
    </form>
    @endif
        ...
        function confirmDelete(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.getElementById('role-delete-' + id).submit();
                }
            });
        }
    </script>
