<div class="flex align-items-center list-business-action">
  <!-- View Action -->
<a class="btn btn-sm btn-icon btn-info" data-bs-toggle="tooltip" title="View Logs" href="{{ route('audit_logs.show', $id) }}">
    ...
    {{-- <!-- Edit Action -->
    <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="tooltip" title="Edit Logs" href="{{ route('audit_logs.edit', $id) }}">
        <span class="btn-inner">
            <svg width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- Delete Action -->
<a class="btn btn-sm btn-icon btn-danger" onclick="confirmDelete('{{ $id }}')" data-bs-toggle="tooltip" title="Delete Log" href="#">
    <span class="btn-inner">
...
<form action="{{ route('auditlogs.destroy', $id) }}" id="audit-delete-{{ $id }}" method="post">
    @method('delete')
    @csrf
</form>
...
<script>
    function confirmDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                document.getElementById('auditlog-delete-' + id).submit();
            }
        });
    }
</script>
    ...
    <form action="{{ route('auditlogs.destroy', $id) }}" id="auditlogs-delete-{{$id}}" method="post">
        @method('delete')
        @csrf
    </form>
