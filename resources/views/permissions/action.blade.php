<div class="flex align-items-center list-business-action">
  <!-- View Action -->
<a class="btn btn-sm btn-icon btn-info" data-bs-toggle="tooltip" title="View Permissions" href="{{ route('permissions.show', $id) }}">
    ...
    <!-- Edit Action -->
    <a class="btn btn-sm btn-icon btn-warning" data-bs-toggle="tooltip" title="Edit Permission" href="{{ route('permissions.edit', $id) }}">
        <span class="btn-inner">
            <svg width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- Delete Action -->
<a class="btn btn-sm btn-icon btn-danger" onclick="confirmDelete('{{ $id }}')" data-bs-toggle="tooltip" title="Delete Permission" href="#">
    <span class="btn-inner">
...
<form action="{{ route('permissions.destroy', $id) }}" id="permission-delete-{{ $id }}" method="post">
    @method('delete')
    @csrf
</form>
    ...
    function confirmDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                document.getElementById('permission-delete-' + id).submit();
            }
        });
    }
</script>
